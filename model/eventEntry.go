...
func _eventEntryInit() {
	_gEventEntryOnce.Do(func() {
		...
		_gEventEntryQueries.EventEntriesByVenueQuery = strings.ReplaceAll(string(sqlFileContent), "{{schema}}", app.GApp.DbConfig.DBSchema)
		...
		_gEventEntryQueries.EventEntriesByOrganizerQuery = strings.ReplaceAll(string(sqlFileContent), "{{schema}}", app.GApp.DbConfig.DBSchema)
	})
}
...
func GetEventEntriesByVenueIdAndUserId(app app.App, c *gin.Context, venueId int, userId int) ([]EventEntry, int) {
	_eventEntryInit()
	var entries []EventEntry
	rows, err := app.DbPool.Query(context.Background(), _gEventEntryQueries.EventEntriesByVenueQuery, venueId, userId)
	if err != nil {
		fmt.Println(err)
...
func GetEventEntriesByOrganizerAndUser(app app.App, c *gin.Context, organizerId int, userId int) ([]EventEntry, int) {
	_eventEntryInit()
	var entries []EventEntry
	rows, err := app.DbPool.Query(context.Background(), _gEventEntryQueries.EventEntriesByOrganizerQuery, organizerId, userId)
	if err != nil {
		fmt.Println(err)
		httpErr := app.DbErrorToHTTP(err)
		return entries, httpErr
