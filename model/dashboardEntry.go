...
func _dashboardEntryInit() {
	_gDashboardEntryOnce.Do(func() {
		...
		_gDashboardEntryQueries.DashboardEntryQuery = strings.ReplaceAll(string(sqlFileContent), "{{schema}}", app.GApp.DbConfig.DBSchema)
...
func GetDashboardEntriesByUserId(app app.App, c *gin.Context, userId int) ([]DashboardEntry, int) {
	_dashboardEntryInit()
	var entries []DashboardEntry
	rows, err := app.DbPool.Query(context.Background(), _gDashboardEntryQueries.DashboardEntryQuery, userId)
	if err != nil {
		fmt.Println(err)
