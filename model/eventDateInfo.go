...
func GetEventDateInfosByUserId(app app.App, ctx *gin.Context, i18n_locale string, userId int) ([]EventDateInfo, error) {
	...
	query := `
		WITH 
			...
			UserPermissions AS (
				SELECT 
					...
					END AS can_edit
				FROM uranus.event e
				JOIN uranus.venue v ON v.id = e.venue_id
				JOIN uranus.user_venue_links uvl ON uvl.venue_id = v.id
				JOIN uranus.event_date ed ON ed.event_id = e.id
				JOIN uranus.user_role ur ON ur.id = uvl.user_role_id
		...
		SELECT 
			...
			up.can_edit
		FROM uranus.event e
		JOIN uranus.event_date ed ON e.id = ed.event_id
		LEFT JOIN uranus.venue v ON v.id = COALESCE(ed.venue_id, e.venue_id)
		LEFT JOIN uranus.space s ON s.id = COALESCE(ed.space_id, e.space_id)
		LEFT JOIN uranus.event_link_types elt ON elt.event_id = e.id
		LEFT JOIN EventTypes et ON et.event_type_id = elt.event_type_id
		LEFT JOIN uranus.genre_link_types glt ON glt.event_id = e.id
		LEFT JOIN GenreTypes gt ON gt.genre_type_id = glt.genre_type_id
		LEFT JOIN uranus.venue_link_types vlt ON vlt.venue_id = v.id
		LEFT JOIN VenueTypes vt ON vt.venue_type_id = vlt.venue_type_id
		LEFT JOIN SpaceTypes st ON st.space_type_id = s.space_type_id
		LEFT JOIN uranus.organizer o ON o.id = e.organizer_id
		LEFT JOIN uranus.event_date_link_images edli ON edli.event_date_id = ed.id AND edli.main_image = TRUE
		LEFT JOIN uranus.event_link_images eli ON eli.event_id = e.id AND eli.main_image = TRUE
		LEFT JOIN uranus.Image i ON i.id = COALESCE(edli.image_id, eli.image_id)
	...
	rows, err := app.DbPool.Query(context.Background(), query, i18n_locale, userId)
	if err != nil {
...
func GetEventDateInfosByUserId2(app app.App, ctx *gin.Context, userId int) ([]EventDateInfo, error) {
	...
	query := `
		...
		FROM
			uranus.event e
		JOIN
			uranus.venue v ON v.id = e.venue_id
		JOIN
			uranus.user_venue_links uvl ON uvl.venue_id = v.id
		JOIN
			uranus.event_date ed ON ed.event_id = e.id
		JOIN
			uranus.user_role ur ON ur.id = uvl.user_role_id
		WHERE
			uvl.user_id = $1
			AND ed.start >= NOW()
		GROUP BY
	...
	rows, err := app.DbPool.Query(context.Background(), query, userId)
	if err != nil {
		log.Printf("Query failed: %v\n", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Query failed"})
		return nil, fmt.Errorf("query failed: %w", err)
