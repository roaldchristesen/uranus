...
func _venueInit() {
	_gVenueOnce.Do(func() {
		...
		_gVenueQueries.VenueQuery = strings.ReplaceAll(_venueQueryTemplate, "{{schema}}", app.GApp.DbConfig.DBSchema)
	})
}
...
func GetVenueById(app app.App, venueId int) (Venue, int) {
	_venueInit()
	// Execute the query to get the venue
	rows, err := app.DbPool.Query(context.Background(), _gVenueQueries.VenueQuery, venueId)
	if err != nil {
		fmt.Println(err)
...
func GetVenuesByUserId(app app.App, ctx *gin.Context, userId int) ([]Venue, error) {
	...
	query := `
		...
		FROM
			uranus.user_venue_links uvl
		JOIN
			uranus.user u ON u.id = uvl.user_id
		JOIN
			uranus.venue v ON v.id = uvl.venue_id
		JOIN
...
func (venue *Venue) GetStats(app app.App, ctx *gin.Context) error {
	...
	query := `
		...
		FROM
			uranus.venue v
		JOIN
			uranus.space s ON s.venue_id = v.id
		LEFT JOIN
			uranus.event e ON e.venue_id = v.id
		LEFT JOIN
	...
	rows, err := app.DbPool.Query(context.Background(), query, venue.Id)
	if err != nil {
		log.Printf("Query failed: %v\n", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Query failed"})
