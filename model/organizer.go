...
func _organizerInit() {
	_gOrganizerOnce.Do(func() {
		...
		_gOrganizerQueries.OrganizerQuery = strings.ReplaceAll(_organizerQueryTemplate, "{{schema}}", app.GApp.DbConfig.DBSchema)
	})
}
...
func GetOrganizerById(app app.App, organizerId int) (Organizer, int) {
	_organizerInit()

	// Execute the query to get the organizer
	rows, err := app.DbPool.Query(context.Background(), _gOrganizerQueries.OrganizerQuery, organizerId)
	if err != nil {
		fmt.Println(err)
...
func GetOrganizersByUserId(app app.App, ctx *gin.Context, userId int) ([]Organizer, error) {
	_organizerInit()
	query := `
		SELECT
		...
		FROM uranus.user_organizer_links AS uol
		JOIN uranus.user AS u ON u.id = uol.user_id
		JOIN uranus.organizer AS uo ON uo.id = uol.organizer_id
		JOIN uranus.user_role AS ur ON ur.id = uol.user_role_id
		WHERE uol.user_id = $1
		ORDER BY
		organizer_name
	`
...
func (organizer *Organizer) GetStats(app app.App, ctx *gin.Context) error {
	...
	LEFT JOIN uranus.event e ON e.venue_id = v.id
	LEFT JOIN uranus.event_date ed ON ed.event_id = e.id
	WHERE o.id = $1
	AND (ed.start >= NOW() OR ed.start IS NULL)`

	rows, err := app.DbPool.Query(context.Background(), query, organizer.Id)
	if err != nil {
		log.Printf("Query failed: %v\n", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Query failed"})
